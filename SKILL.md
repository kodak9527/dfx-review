...
| **低** | 已读文件 <30%，或仅读 Entity/DTO | 最终评分 ×0.7 折扣，报告中标注"采样不足" |
...
|---|--------|--------------------------|---------|
| 1 | **日志框架** | `@Slf4j`、`Logger` 声明、`log.info/error/warn/debug` | 2.1 日志诊断 |
| 2 | **日志上下文** | `MDC.put("traceId"`、`MDC.put("orderId"`、JSON encoder 配置 | 2.1 日志诊断 |
| 3 | **全局异常处理** | `@RestControllerAdvice`、`@ExceptionHandler`、`BusinessException` 定义 | 2.2 异常处理 |
| 4 | **统一错误响应** | `ErrorResponse`、`ApiError` DTO 定义 | 2.2 异常处理 |
| 5 | **指标埋点** | `@Timed`、`Counter.builder`、`Timer.builder`、`MeterBinder` | 2.3 指标监控 |
| 6 | **健康检查** | `HealthIndicator`、`AbstractHealthIndicator`、`livenessState/readinessState` | 2.4 健康检查 |
| 7 | **熔断降级** | `@CircuitBreaker(`、`@Bulkhead(`、`@RateLimiter(`、`fallbackMethod`、`@Retryable(` | 2.5 熔断与韧性 |
| 8 | **缓存** | `@Cacheable(`、`@CacheEvict(`、`@CachePut(`、`CacheManager`、`RedisCacheManager` | 2.6 缓存 |
| 9 | **异步执行** | `@Async(`、`ThreadPoolTaskExecutor`、`setCorePoolSize`、`RejectedExecutionHandler` | 2.9 异步线程池 |
| 10 | **链路追踪** | `io.opentelemetry`、`@WithSpan(`、`brave.Tracer`、`tracer.nextSpan()` | 2.8 链路追踪 |
| 11 | **配置属性** | `@ConfigurationProperties(`、`@RefreshScope`、`@Validated`、`@ConditionalOnProperty(` | 2.10 配置管理 |
| 12 | **API 版本化** | `/v[0-9]+/` 路径、`X-API-Version` 头、`@Deprecated` | 2.11 API 版本化 |
| 13 | **优雅关闭** | `server.shutdown=graceful`、`@PreDestroy`、`timeout-per-shutdown-phase` | 2.12 优雅关闭 |
| 14 | **安全防护** | `RateLimiter` 过滤器、`@CrossOrigin`、`CorsConfigurationSource`、`MODE_INHERITABLETHREADLOCAL` | 2.13 安全 DFX |
| 15 | **幂等性** | `X-Idempotency-Key`、`SETNX` 原子操作、Redis 幂等键 | 3.1 订单幂等 |
| 16 | **库存锁** | `SELECT FOR UPDATE`、`@Lock(`、`Redisson`/`RedisLock`、分布式锁 | 3.2 库存锁定 |
| 17 | **支付状态机** | 支付状态枚举（UNPAID/PAYING/PAID）、`outTradeNo`、`verifySign` | 3.3 支付事务 |
| 18 | **分布式事务** | `@GlobalTransactional`、`@Saga`、`Outbox`、`@TransactionalEventListener` | 3.4 分布式事务 |
| 19 | **秒杀防护** | Redis 预加载、MQ 队列、`@RateLimiter` 用户级限流、秒杀模块降级 | 3.5 秒杀防护 |
| 20 | **状态机** | `StateMachine`、`squirrel-foundation`、`stateless4j`、`@Scheduled` 卡单检测 | 3.6 状态机监控 |
| 21 | **第三方降级** | `interface.*Client`/`interface.*Gateway` 抽象、`@CircuitBreaker` 每服务独立 | 3.7 第三方降级 |
| 22 | **业务告警** | `Counter.builder` 含业务事件名、`alert` 前缀配置、`SLA/SLI` 计算 | 3.8 业务告警 |
| 23 | **漏斗追踪** | `AnalyticsEvent`/`FunnelEvent`/`UserActionEvent`、转化节点事件、漏斗指标 | 3.9 漏斗追踪 |
| 24 | **超时管理** | `@Transactional(timeout=N)`、`RestTemplate`/`WebClient` 超时配置、`@TimeLimiter(` | 3.10 超时管理 |
| 25 | **数据对账** | `@Scheduled` 对账任务、`reconcil`/`settlement` 类名、跨系统比对 | 3.11 数据一致性 |
...
| 用户要求中止 | 立即停止扫描，输出部分报告（如有），提示"可重新触发继续扫描"
